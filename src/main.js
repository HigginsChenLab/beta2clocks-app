...
const show = (id) => {
  ["view-engine", "view-setup", "view-run", "view-report"].forEach((v) =>
...
function setPill(kind, text) {
  const colors = {
    ok: ["bg-green-50 text-green-700", "bg-green-500"],
    warn: ["bg-amber-50 text-amber-700", "bg-amber-500"],
    bad: ["bg-red-50 text-red-600", "bg-red-500"],
    idle: ["bg-gray-100 text-gray-500", "bg-gray-300"],
  }[kind];
...
function engineCard(html) {
  $("engine-card").innerHTML = html;
}
...
async function checkEngine() {
  ...
  engineCard(`
    <div class="text-center py-6">
      <div class="text-blue-600 flex justify-center mb-3">${icon.spinner}</div>
      <p class="text-gray-600">Checking your Docker installation…</p>
    </div>`);
  ...
  if (!status.installed) {
    ...
    engineCard(`
      <div class="text-center">
        <h2 class="text-lg font-semibold text-gray-900">Docker is required</h2>
        <p class="text-gray-500 mt-2 text-sm max-w-md mx-auto">beta2clocks runs the analysis inside a Docker container so your results exactly match the TranslAGE pipeline. Install Docker Desktop (free), then come back.</p>
        <div class="flex items-center justify-center gap-3 mt-6">
          <a href="${DOCKER_URL}" target="_blank" rel="noopener noreferrer" class="bg-blue-600 text-white px-5 py-2.5 rounded-lg font-semibold hover:bg-blue-700 transition-colors">Get Docker Desktop</a>
          <button id="btn-recheck" class="border-2 border-gray-300 text-gray-600 px-5 py-2.5 rounded-lg font-semibold hover:bg-gray-50 transition-colors">Re-check</button>
        </div>
      </div>`);
  ...
  if (!status.running) {
    ...
    engineCard(`
      <div class="text-center">
        <h2 class="text-lg font-semibold text-gray-900">Start Docker Desktop</h2>
        <p class="text-gray-500 mt-2 text-sm max-w-md mx-auto">${status.message || "Docker is installed but the engine isn't running. Open Docker Desktop, wait for it to say “running”, then re-check."}</p>
        <button id="btn-recheck" class="mt-6 bg-blue-600 text-white px-5 py-2.5 rounded-lg font-semibold hover:bg-blue-700 transition-colors">Re-check</button>
      </div>`);
    $("btn-recheck").onclick = checkEngine;
    return;
  }
  ...
  if (!hasImage) {
    engineCard(`
      <div class="text-center">
        <h2 class="text-lg font-semibold text-gray-900">Download the clock engine</h2>
        <p class="text-gray-500 mt-2 text-sm max-w-md mx-auto">One-time download of the beta2clocks engine image (a few GB). This contains R, methylCIPHER and all clock reference data, so everything afterwards runs offline.</p>
        <button id="btn-pull" class="mt-6 bg-gradient-to-r from-blue-600 to-purple-700 text-white px-6 py-2.5 rounded-lg font-semibold hover:from-blue-700 hover:to-purple-800 transition-all">Download engine</button>
        <div id="pull-status" class="hidden mt-5 text-left">
          <div class="flex items-center gap-2 text-sm text-gray-600 mb-2"><span class="text-blue-600">${icon.spinner}</span><span id="pull-line" class="truncate">Starting download…</span></div>
          <div class="relative h-2 rounded-full bg-gray-100 overflow-hidden indeterminate-bar"></div>
        </div>
      </div>`);
...
async function chooseInput() {
  ...
  // Update the input button label.
  const name = selected.split("/").pop();
  $("btn-choose-input").innerHTML = `
    <svg class="w-7 h-7 mx-auto text-blue-500" fill="none" stroke="currentColor" stroke-width="1.7" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 4H7a2 2 0 01-2-2V6a2 2 0 012-2h7l5 5v9a2 2 0 01-2 2z"/></svg>
    <span class="block mt-2 font-medium text-gray-800 selectable">${name}</span>
    <span class="block text-xs text-blue-500 mt-0.5">Click to choose a different file</span>`;
...
async function runPreflight() {
  ...
  box.innerHTML = `<div class="flex items-center gap-2 text-sm text-gray-500 rounded-lg bg-gray-50 px-4 py-3"><span class="text-blue-600">${icon.spinner}</span> Spot-checking your data…</div>`;
  ...
  try {
    ...
    box.innerHTML = `<div class="rounded-lg bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-700"><span class="font-semibold">Couldn't read the file.</span><pre class="selectable mt-1 whitespace-pre-wrap text-xs text-red-600">${String(e)}</pre></div>`;
    setSetupEnabled(false);
    return;
  }
...
function renderPreflight(r) {
  ...
  const checks = (r.checks || [])
    .map((c) => {
      const color = c.pass ? "text-green-600" : "text-red-500";
      const ic = c.pass ? icon.check : icon.x;
      return `<li class="flex items-start gap-2 py-1"><span class="${color} mt-0.5 shrink-0">${ic}</span><span class="text-sm text-gray-600"><span class="font-medium text-gray-800">${c.name}.</span> ${c.message}</span></li>`;
    })
    .join("");

  const stat = (label, val) =>
    `<div><p class="text-xs text-gray-400">${label}</p><p class="text-sm font-semibold text-gray-900">${val}</p></div>`;
  const stats = [
  ...
  const banner = r.ok
    ? `<div class="flex items-center gap-2 rounded-t-lg bg-green-50 border border-green-200 px-4 py-3 text-green-700"><span>${icon.check}</span><span class="text-sm font-semibold">Looks good — ready to run.</span></div>`
    : `<div class="flex items-center gap-2 rounded-t-lg bg-red-50 border border-red-200 px-4 py-3 text-red-700"><span>${icon.x}</span><span class="text-sm font-semibold">This file can't be processed yet. See below.</span></div>`;

  box.innerHTML = `
    ${banner}
    <div class="border border-t-0 border-gray-200 rounded-b-lg px-4 py-3">
      ${stats ? `<div class="grid grid-cols-2 sm:grid-cols-4 gap-3 pb-3 mb-2 border-b border-gray-100">${stats}</div>` : ""}
      <ul>${checks}</ul>
    </div>`;
...
function renderReport(r) {
  ...
  let tone, ic, title, sub;
  if (r.cancelled) {
    tone = "bg-gray-100 text-gray-700";
    ic = `<div class="text-gray-500">${icon.x}</div>`;
    title = "Run cancelled";
    sub = "No results were saved.";
    ...
    title = "Finished with some failures";
    sub = r.message;
  } else {
    tone = "bg-red-50 text-red-800 border border-red-200";
    ic = `<div class="w-10 h-10 rounded-full bg-red-500 text-white flex items-center justify-center">${icon.x}</div>`;
    title = "Run did not finish";
    sub = r.message;
  }
  hero.className = "rounded-xl p-6 mb-5 flex items-center gap-4 " + tone;
  hero.innerHTML = `${ic}<div><p class="text-lg font-bold">${title}</p><p class="text-sm opacity-80">${sub}</p></div>`;

  const okCount = (r.succeeded || []).length;
  const failCount = (r.failed || []).length;
  $("rep-time").textContent =
    r.total_minutes != null ? `${r.total_minutes} min` : "—";
  $("rep-ok").textContent = okCount || "—";
  $("rep-fail").textContent = failCount;
  $("rep-fail").className =
    "text-xl font-semibold " + (failCount > 0 ? "text-red-600" : "text-gray-900");
  ...
  const okChips = (r.succeeded || []).map(
    (n) =>
      `<span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-green-50 text-green-700 text-xs font-medium"><span class="text-green-500">${icon.check}</span>${n}</span>`,
  );
  const failChips = (r.failed || []).map(
    (n) =>
      `<span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-red-50 text-red-600 text-xs font-medium"><span class="text-red-400">${icon.x}</span>${n}</span>`,
  );
  chips.innerHTML =
    failChips.join("") + okChips.join("") ||
    `<span class="text-sm text-gray-400">No clock results were reported.</span>`;
}
...
function resetForAnother() {
  ...
  $("btn-choose-input").innerHTML = `
    <svg class="w-8 h-8 mx-auto text-gray-400 group-hover:text-blue-500 transition-colors" fill="none" stroke="currentColor" stroke-width="1.7" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 16V4m0 0L8 8m4-4l4 4M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2"/></svg>
    <span class="block mt-2 font-medium text-gray-700 group-hover:text-blue-600">Choose a <code>.RData</code> file</span>
    <span class="block text-xs text-gray-400 mt-0.5">A beta matrix (<code>DNAm…</code>) + phenotypes (<code>pheno…</code>)</span>`;
  setSetupEnabled(false);
  goToSetup();
}
